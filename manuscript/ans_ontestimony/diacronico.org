...
**** El Argumento
#+BEGIN_SRC latex
  Los razonamientos Parmenidianos ofrecen muchas oportunidades de reflexión a
  Elizabeth. La argumentación que utilizará en su análisis la sintetiza como
  sigue:
  ...
  \textelp{} \textgreek{οὐ γὰρ φατὸν οὐδὲ νοητόν ἔστιν ὅπως οὐκ ἔστι.} (VIII)]{
    ...
    $\therefore$ Lo que no es no puede ser pensado} También plantea esta otra
  manera de expresar el mismo argumento:
  ...
  atenta podría dar la impresión de que el argumento consiste en:
  ...
  premisa de cada argumento, las proposiciones \enquote{Lo que no es no puede ser}
  o \enquote{Lo que es nada no puede ser}, están basadas en que \enquote{Lo que no
  ...
  \blockquote[{\cite[vii]{anscombe1981parmenides}}: \textins{doesn't} derive the
  ...
  Concerning that which is not, it holds that \emph{that} cannot be]{Concerniendo
    aquello que no es, se sostiene que eso no puede ser}.

  Ahora bien, Anscombe considera que para que el argumento sea valido esta segunda
  premisa tiene que ser entendida \emph{in sensu diviso}. Sin embargo, entendida
  así, esta premisa no es creíble.

  Ahora bien, la premisa representa varias acepciones según se entienda como una
  afirmación general (in sensu composito) o como una afirmación particular (in
  sensu diviso).

  La imposibilidad de lo que no es, no es solo la imposibilidad de la proposición
  ``lo que no es, es'' es decir, la verdad de ``Lo que no es no puede ser'',
  tomado en sensu composito. Eso puede ser dejado a un lado como irrelevante. Lo
  que no es, ciertamente no puede estar siendo, pero puede llegar a ser, y en
  este sentido lo que no es es posible. Cuando haya llegado a ser, ciertamente ya
  no es lo que no es, así que en llamarlo posible no estamos declarando que ``Lo
  que no es, es'' es posible. Así que no puede mostrarse como imposible que pueda
  llegar a ser solo por señalar la imposibilidad de que este siendo.
  Pero esta no puede ser toda la historia. Que lo que no es es nada implica que no
  hay nada ahí para llegar a ser. Así ``Lo que no es puede ser'' tomado en sensu
  diviso, digase como: ``Concerniendo lo que no es, eso puede ser'' es acerca de
  nada en  absoluto. Si fuera acerca de algo, entonces sería sobre algo que no es,
  y así habría un ejemplo de ``Lo que no es, es'' que sería verdadero.
