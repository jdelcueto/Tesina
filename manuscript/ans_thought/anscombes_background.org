...
*** DONE Vida salvaje luchando por emerger abiertamente
   ...
   abiertamente'',\autocite[cf.˜][loc.˜]{monk1991duty} quedaba expresada en su quehacer
   filosífico.
    #+END_SRC
...
** Derrota de la concepción representativa del lenguaje
   ...
   #+BEGIN_SRC latex 
     ...
     Wittgenstein se planteaba lo siguiente:

     ¿Cómo se que al calcular la serie $+$2 debo escribir ``20004, 20006'' y no
     ``20004, 20008''? (La pregunta ``¿Cómo se que este color es `rojo'?'' es
     similar.)

     cómo se qué hacer con este conocimiento previo cuando el paso
     tiene que ser actualmente tomado?
     dudas de qué tienes que hacer después de 2004? no
     este no tener duda no significa que ha sido determinado de antemano, sino que se
     que cualquier numero que se me de seré capaz de del sel siguiete con certeza

     y en qué consiste esta certeza?
     ...
     \citalitlar{
       ...
       cualquiera puede contar en el orden que quiera?>> ---Probablemenre no lo
       llamariamos `contar' si todo el mundo dijera los números uno después de otro
       ...
       insistido que hemos de decir `dos' después de `uno', `tres' despues de `dos' y
       ...
       verdad' significa: ser usable (o útil)?>> ---No, no eso; pero que no puede ser
       dicho de la serie de números naturales --y tampoco de nuestro lenguaje-- que
       es verdad, pero: que es usable, y, sobre todo que \emph{se usa de
